...
var Util = {
	...
	getFileList:function (){
		...
		var pageDir = path.resolve(__dirname,'../packages')
		if(!fs.existsSync(pageDir)){
			throw new Error("can't find pages directory.")
		}
		var files = fs.readdirSync(pageDir)
		...
		files = files.filter(function(f){
			return fs.existsSync(pageDir+'/'+f+'/src/app.js')
		})
		// if(files.length === 0){
		// 	throw new Error("can't find any page")
		// }
	...
	getResolve:function (resolve){
		...
		for(var i = 0 ; i < files.length; i++){
			fname = files[i]
			resolve[fname] = 'packages/'+fname+'/src';
		}
		return resolve
	},
	getPages : function (lib,toArray,env,filters){
		...
		for(var i = 0; i < files.length; i++){
			...
			var entryName = isDev ? fname+'/app' : upper(fname);
			if(toArray){
				entries[entryName] = ["./packages/"+fname+"/"+targetDir+"/app.js"]
			}else{
				entries[entryName] = "./packages/"+fname+"/"+targetDir+"/app.js"
			}
			let chunkArr = []
			plugins.push(new HtmlWebpackPlugin({
		      filename:(isDev ? fname+'/index' : entryName) +'.html',
		      template:'packages/'+fname+'/'+targetDir+'/index.html',
		      inject:'body',
